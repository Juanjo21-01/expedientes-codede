...
<div>
    <x-patterns.responsive-table title="Listado de Municipios" :count="$this->municipios->total()" tone="base">
        <x-slot:head>
        ...
        @forelse ($this->municipios as $index => $municipio)
            <tr class="hover">
                    ...
                    @if ($municipal || $tecnicos->isNotEmpty())
                        <div class="flex flex-col items-center gap-0.5">
                            ...
                            @if ($tecnicos->isNotEmpty())
                                <span class="badge badge-xs badge-soft badge-info badge-outline"
                ...
                <td class="text-center">
                    <div class="flex flex-col items-center gap-0.5">
                        @if ($municipio->expedientes_count > 0)
                            <span class="badge badge-sm badge-soft badge-outline">
                                {{ $municipio->expedientes_count }} total
                            </span>
                        @endif
                        @if ($municipio->expedientes_activos_count > 0)
                            <span class="badge badge-sm badge-soft badge-primary badge-outline">
                ...
                <td class="text-center">
                    @if (auth()->user()->isAdmin())
                        <div class="tooltip" data-tip="Cambiar estado">
                            <button wire:click="cambiarEstado({{ $municipio->id }})"
                                class="badge badge-sm badge-soft cursor-pointer transition-all hover:scale-105 gap-1 {{ $municipio->estaActivo() ? 'badge-success' : 'badge-error' }}">
                                <div
                    ...
                    @else
                        <span
                            class="badge badge-sm badge-soft gap-1 {{ $municipio->estaActivo() ? 'badge-success' : 'badge-error' }}">
                            <div
                                class="status {{ $municipio->estaActivo() ? 'status-success' : 'status-error' }} status-xs">
                            </div>
                            {{ $municipio->estaActivo() ? 'Activo' : 'Inactivo' }}
