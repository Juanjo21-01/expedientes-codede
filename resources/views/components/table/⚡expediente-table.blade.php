...
<div>
    <x-patterns.responsive-table title="Listado de Expedientes" :count="$this->expedientes->total()" tone="base">
        <x-slot:head>
        ...
        @forelse ($this->expedientes as $index => $expediente)
            <tr class="hover:bg-base-200/50 transition-colors">
                ...
                <td class="text-center">
                    <span
                        class="badge badge-sm badge-soft {{ $expediente->estado_badge_class }} whitespace-nowrap font-medium">
                        {{ $expediente->estado }}
                    </span>
                </td>
                        ...
                        @can('update', $expediente)
                            <div class="tooltip" data-tip="Editar">
                                <a href="{{ route('expedientes.edit', $expediente->id) }}"
                                    class="btn btn-ghost btn-xs btn-square text-warning hover:bg-warning/10" wire:navigate>
                        ...
                        @can('revisarFinanciera', $expediente)
                            <div class="tooltip" data-tip="Revisar">
                                <a href="{{ route('expedientes.revision', $expediente->id) }}"
                                    class="btn btn-ghost btn-xs btn-square text-accent hover:bg-accent/10" wire:navigate>
                        ...
                        @can('cambiarEstado', $expediente)
                            <div class="tooltip" data-tip="Cambiar estado">
                                <button @click="$dispatch('abrir-modal-estado',  {expedienteId: {{ $expediente->id }} })"
                                    class="btn btn-ghost btn-xs btn-square text-secondary hover:bg-secondary/10">
